 ...
 * @retval      无
 */
void usb_get_value_from_nvs(const char *key, void *value, size_t *size)
{
 ...
 */
esp_err_t usb_set_value_to_nvs(const char *key, void *value, size_t size)
{
    nvs_handle_t my_handle;
    esp_err_t err = nvs_open("memory", NVS_READWRITE, &my_handle);
...
USB_Esp32Camera::~USB_Esp32Camera() {
    ...
    if (decode_frame_buffer) {
        heap_caps_free(decode_frame_buffer);
